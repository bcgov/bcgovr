...
`use_bcgov_github()`  Open a [bcgov GitHub repository](https://github.com/bcgov) and synchronise with a git-initialised R project _and_ add files that ensure the project meets [bcgov GitHub requirements](https://github.com/bcgov/BC-Policy-Framework-For-GitHub/blob/master/BC-Gov-Org-HowTo/Cheatsheet.md).
...
`r emo::ji("tada")` The authors of `bcgovr` acknowledge and thank [RStudio](https://www.rstudio.com/) and the authors of the [`usethis`](https://cran.r-project.org/web/packages/usethis/index.html) R package&mdash;`bcgovr` uses `usethis` _a lot_. And by a lot, we mean _every_ `bcgovr` function uses `usethis` under the hood `r emo::ji("oncoming_automobile")`.
...
To add your project to bcgov GitHub with `use_bcgov_github()` users need to have a GitHub account _and_ be a [member of the bcgov GitHub organisation](https://github.com/bcgov/BC-Policy-Framework-For-GitHub/blob/master/BC-Gov-Org-HowTo/Cheatsheet.md). 
